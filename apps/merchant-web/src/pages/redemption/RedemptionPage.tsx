...
import {
  ...
  Descriptions, InputNumber, message, Alert, Row, Col, Statistic,
} from 'antd'
...
// Scan response shape from backend:
// { orderItemId, productName, variantName, totalQuantity, redeemedCount, remainingCount, validUntil, orderId }
...
export default function RedemptionPage() {
  ...
  const logColumns = [
    {
      title: 'Code',
      dataIndex: ['orderItem', 'redemptionCode'],
      key: 'code',
      ...
      <Row gutter={24}>
        <Col span={12}>
          <Card title={<Space><QrcodeOutlined /> Scan Code</Space>}>
            ...
            {previewError && (
              <Alert type="error" message={previewError} showIcon style={{ marginBottom: 16 }} />
            )}
              ...
              <div>
                ...
                <Descriptions column={1} bordered size="small" style={{ marginBottom: 16 }}>
                  ...
                  <Descriptions.Item label="Remaining">
                    <Text strong style={{ color: '#52c41a' }}>{preview.remainingCount}</Text>
                  </Descriptions.Item>
                  <Descriptions.Item label="Expires">
                    {preview.validUntil ? dayjs(preview.validUntil).format('YYYY-MM-DD') : 'No expiry'}
                  </Descriptions.Item>
                </Descriptions>
                ...
                <div>
                  ...
                  <Space>
                    <InputNumber
                      min={1}
                      max={preview.remainingCount}
                      value={confirmQty}
                      onChange={(v) => setConfirmQty(v ?? 1)}
                    />
                    <Button
                      type="primary"
                      size="large"
        ...
        <Col span={12}>
          <Card>
            <Row gutter={16}>
              <Col span={12}>
                <Statistic title="Total Redemptions" value={logsTotal} prefix={<QrcodeOutlined />} />
              </Col>
              <Col span={12}>
                <Statistic
                  title="Today"
                  value={logs.filter((l) => dayjs(l.createdAt).isAfter(dayjs().startOf('day'))).length}
                />
              </Col>
            </Row>
          </Card>
      ...
      <Card title="Redemption History" style={{ marginTop: 24 }}>
        <Table
          ...
          pagination={{
            ...
            pageSize: 10,
            current: logsPage,
            onChange: (p) => { setLogsPage(p); fetchLogs(p) },
          }}
          size="small"
        />
      </Card>
