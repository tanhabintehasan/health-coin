...
import {
  ...
  Select, Typography, Popconfirm, message, Row, Col, Slider,
} from 'antd'
...
export default function ProductsPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'deliveryType',
      width: 130,
      render: (v: string) => <Tag>{v === 'IN_STORE_REDEMPTION' ? 'In-Store' : 'Delivery'}</Tag>,
    },
    {
      title: 'Price',
      key: 'basePrice',
    ...
    },
    {
      ...
      width: 110,
      render: (v: string) => (
        <Tag color={v === 'ACTIVE' ? 'green' : v === 'PENDING_REVIEW' ? 'orange' : 'default'}>{v}</Tag>
      ),
    },
    {
      title: 'Actions',
    ...
    <div>
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          <Row gutter={16}>
            ...
            <Col span={8}>
              <Form.Item name="productType" label="Type" rules={[{ required: true }]}>
                <Select options={PRODUCT_TYPE_OPTIONS} disabled={!!editing} />
              </Form.Item>
            </Col>
          </Row>
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>
              <Form.Item name="categoryId" label="Category">
                <Select options={categories} placeholder="Select category" allowClear showSearch />
              </Form.Item>
            </Col>
          </Row>
          ...
          <Form.Item
            name="coinOffsetRate"
            label={`HealthCoin Offset Rate: ${Math.round((form.getFieldValue('coinOffsetRate') ?? 0) * 100)}%`}
          >
          ...
          {productType === 'SERVICE' && (
            <Form.Item name="validityDays" label="Validity (days after purchase)">
