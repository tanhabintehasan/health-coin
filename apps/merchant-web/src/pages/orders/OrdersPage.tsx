...
import {
  Card, Table, Tag, Select, Space, Typography, Drawer, Descriptions,
  Button, Steps, message, Divider,
} from 'antd'
...
// OrderStatus enum: PENDING_PAYMENT | PAID | PROCESSING | SHIPPED | COMPLETED | CANCELLED | REFUNDING | REFUNDED
const ALL_STATUSES = [
...
// Valid merchant-driven transitions (mirrors backend order-state-machine)
  ...
  const columns = [
    {
      ...
      width: 130,
      render: (v: string) => (
        <Button type="link" style={{ padding: 0 }} onClick={() => setSelected(orders.find((o) => o.id === v))}>
      ...
      <Drawer
        ...
        extra={
          NEXT_STATUS[selected?.status] && (
            <Button type="primary" loading={advancing} onClick={advanceStatus}>
      ...
      >
        {selected && (
          <Space direction="vertical" style={{ width: '100%' }} size={16}>
            <Tag color={STATUS_COLOR[selected.status] ?? 'default'} style={{ fontSize: 14, padding: '4px 12px' }}>
            ...
            <Descriptions column={1} size="small" bordered>
              ...
              {selected.paidAt && (
                <Descriptions.Item label="Paid">
                  {dayjs(selected.paidAt).format('YYYY-MM-DD HH:mm')}
                </Descriptions.Item>
              )}
              {selected.deliveryAddress && (
                <Descriptions.Item label="Address">{selected.deliveryAddress}</Descriptions.Item>
              )}
              {selected.note && (
              ...
              {selected.items?.map((item: any) => (
                ...
                >
                  <div>
                    ...
                    <br />
                    <Text type="secondary" style={{ fontSize: 12 }}>{item.variantName} × {item.quantity}</Text>
                    {item.redemptionCode && (
                      <div>
                        <Text code style={{ fontSize: 12 }}>{item.redemptionCode}</Text>
                      </div>
                    )}
                  </div>
