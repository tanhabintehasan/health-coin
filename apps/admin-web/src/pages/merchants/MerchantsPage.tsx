...
import { Table, Tag, Button, Typography, Space, message, Popconfirm, Tabs, Drawer, Descriptions } from 'antd'
...
export default function MerchantsPage() {
  ...
  const fetchMerchants = async (status = activeTab, p = 1) => {
    ...
    try {
      ...
      const res: any = await client.get(`/admin/merchants?${params}`)
  ...
  const handleApprove = async (id: string) => {
    try {
      await client.patch(`/admin/merchants/${id}/approve`)
  ...
  const handleReject = async (id: string) => {
    try {
      await client.patch(`/admin/merchants/${id}/reject`)
  ...
  const handleSuspend = async (id: string, suspend: boolean) => {
    try {
      await client.patch(`/admin/merchants/${id}/suspend`, { suspend })
      message.success(suspend ? 'Merchant suspended' : 'Merchant reactivated')
      fetchMerchants()
      if (selected?.id === id) setSelected(null)
  ...
  return (
    <div>
      ...
      <Tabs
        activeKey={activeTab}
        onChange={(key) => { setActiveTab(key); setPage(1); fetchMerchants(key, 1) }}
      ...
      <Table
        ...
        pagination={{
          ...
          onChange: (p) => { setPage(p); fetchMerchants(activeTab, p) },
        }}
      />
        ...
        {selected && (
          <>
            <Descriptions column={1} bordered size="small">
              ...
              {selected.approvedAt && (
                <Descriptions.Item label="Approved">{dayjs(selected.approvedAt).format('YYYY-MM-DD HH:mm')}</Descriptions.Item>
              )}
              {selected.rejectionNote && (
                <Descriptions.Item label="Rejection Note">{selected.rejectionNote}</Descriptions.Item>
              )}
            ...
            <div style={{ marginTop: 16 }}>
              <Space>
                {selected.status === 'PENDING' && (
                  <>
                    <Popconfirm title="Approve?" onConfirm={() => handleApprove(selected.id)}>
                      <Button type="primary" icon={<CheckOutlined />}>Approve</Button>
                    </Popconfirm>
                    <Popconfirm title="Reject?" onConfirm={() => handleReject(selected.id)}>
