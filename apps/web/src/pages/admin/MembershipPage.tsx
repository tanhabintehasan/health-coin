...
export default function MembershipPage() {
  ...
  const columns = [
    { title: 'Level', dataIndex: 'level', key: 'level', width: 70, render: (v: number) => <Tag color="blue">L{v}</Tag> },
    { title: 'Name', dataIndex: 'name', key: 'name' },
    { title: 'Upgrade Threshold (Mutual Coins)', dataIndex: 'minCoins', key: 'minCoins', render: (v: any) => (Number(v) / 100).toLocaleString() },
    { title: 'Regional Rate (m%)', dataIndex: 'regionalCoinRate', key: 'regionalCoinRate', render: (v: any) => {
      const pct = (Number(v) * 100).toFixed(0)
      return pct === '0' ? <Tag color="default">0% (no regional reward)</Tag> : <Tag color="purple">{pct}%</Tag>
    }},
    { title: 'Description', dataIndex: 'description', key: 'description', ellipsis: true },
    { title: 'Edit', key: 'action', width: 80, render: (_: any, r: any) => <Button size="small" icon={<EditOutlined />} onClick={() => openEdit(r)}>Edit</Button> },
  ]
    ...
    <div>
      ...
      <Modal title={`Edit Tier L${editTarget?.level} — ${LEVEL_NAMES[editTarget?.level] ?? ''}`} open={!!editTarget} onOk={handleSave} onCancel={() => setEditTarget(null)} confirmLoading={saving} okText="Save Changes" width={520}>
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          ...
          <Form.Item name="minCoins" label="Upgrade Threshold (Mutual Coins)" extra="Lifetime mutual coins required to reach this tier automatically" rules={[{ required: true }]}>
            <InputNumber min={0} style={{ width: '100%' }} precision={2} placeholder="e.g. 1000" />
          </Form.Item>
          <Form.Item name="regionalCoinRate" label="Regional Coin Rate — m% (0 for levels 1–2)" extra="Percentage of regional members' spending awarded as Universal Coins. Set 0 for non-agent tiers.">
