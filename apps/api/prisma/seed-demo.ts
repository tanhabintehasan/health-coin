...
export default async function seed(prisma: PrismaClient) {
  ...
  const regionSh = await prisma.region.upsert({
    where: { code: '310000' },
    update: {},
    create: { id: 'prov-sh', name: '上海市', code: '310000', level: 1 },
  })
  const regionBj = await prisma.region.upsert({
    where: { code: '110000' },
    update: {},
    create: { id: 'prov-bj', name: '北京市', code: '110000', level: 1 },
  })

  // ── 2. Demo Users ────────────────────────────────────────────────────────────
  const usersData = [
    { phone: '13800000001', nickname: '管理员', level: 1 },
    { phone: '13800000002', nickname: '张商户', level: 2 },
    { phone: '13800000003', nickname: '李商户', level: 2 },
  ...
  for (let i = 0; i < 12; i++) {
    ...
    await prisma.order.create({
      data: {
        ...
        cashPaid: status !== 'PENDING_PAYMENT' ? BigInt(total) : BigInt(0),
        healthCoinPaid: BigInt(0),
        mutualCoinPaid: BigInt(0),
        universalCoinPaid: BigInt(0),
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const existing = await prisma.withdrawal.findFirst({ where: { userId: user.id, amount } })
