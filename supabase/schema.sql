...
DO $$
BEGIN
  BEGIN CREATE TYPE "WalletType" AS ENUM ('HEALTH_COIN', 'MUTUAL_HEALTH_COIN', 'UNIVERSAL_HEALTH_COIN'); EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN CREATE TYPE "TxType" AS ENUM ('ORDER_REWARD', 'REFERRAL_L1_REWARD', 'REFERRAL_L2_REWARD', 'REGIONAL_REWARD', 'ORDER_PAYMENT', 'WITHDRAWAL', 'REFUND', 'ADMIN_ADJUSTMENT'); EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN CREATE TYPE "MerchantStatus" AS ENUM ('PENDING', 'APPROVED', 'REJECTED', 'SUSPENDED'); EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN CREATE TYPE "ProductType" AS ENUM ('PHYSICAL', 'SERVICE'); EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN CREATE TYPE "DeliveryType" AS ENUM ('DELIVERY', 'IN_STORE_REDEMPTION'); EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN CREATE TYPE "ProductStatus" AS ENUM ('DRAFT', 'PENDING_REVIEW', 'ACTIVE', 'INACTIVE'); EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN CREATE TYPE "OrderStatus" AS ENUM ('PENDING_PAYMENT', 'PAID', 'PROCESSING', 'SHIPPED', 'COMPLETED', 'CANCELLED', 'REFUNDING', 'REFUNDED'); EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN CREATE TYPE "PayoutMethod" AS ENUM ('BANK', 'ALIPAY', 'WECHAT'); EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN CREATE TYPE "WithdrawalStatus" AS ENUM ('PENDING', 'APPROVED', 'PROCESSING', 'COMPLETED', 'REJECTED'); EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN CREATE TYPE "AdminRole" AS ENUM ('SUPER_ADMIN', 'ADMIN', 'FINANCE', 'STAFF'); EXCEPTION WHEN duplicate_object THEN NULL; END;
END $$;
